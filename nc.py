...
VENDAS_CACHE_TTL = int(os.getenv("VENDAS_CACHE_TTL", "300"))
_VENDAS_CACHE = {"timestamp": 0.0, "result": None}
...
def extrair_cpfs(json_data):
    """Extrai todos os CPFs encontrados no JSON de resposta com baixo custo de CPU."""
    cpfs = []
    vistos = set()
    fila = deque([json_data])
...
def vendas_clt():
    """Função principal que consulta as vendas CLT com cache em memória."""
    now = time()
    cached = _VENDAS_CACHE.get("result")
    if cached is not None and (now - _VENDAS_CACHE.get("timestamp", 0.0)) < VENDAS_CACHE_TTL:
        return cached
    ...
    try:
        ...
        if cpfs and len(cpfs) > 0:
            result = {"status": "success", "cpfs": cpfs}
        else:
            result = {"status": "success", "cpfs": []}

        _VENDAS_CACHE["timestamp"] = now
        _VENDAS_CACHE["result"] = result
        return result
